...
foodProcessingComputation <- function(SUAinput, treeNewER, primary){
  ...
  if(nrow(secondLevelProcessing) > 0) {
    ...
    add2NotCovered <- toDeviate2proc2[availability_secondary_primEq_tot*1.001 < (-1)*secondLevelProcessing ]
    ...
    insufficiency <- checkingTab[availability_secondary_primEq_tot*1.001 < (-1)*secondLevelProcessing ]
