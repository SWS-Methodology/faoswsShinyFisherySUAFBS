...
foodProcessingComputation <- function(SUAinput, treeNewER, primary){
  ...
  if(nrow(SUAinput[measuredElementSuaFbs %in% c('5510', '5302') &
    ...
    if(nrow(secondLevelProcessing) > 0) {
      ...
      if(nrow(toDeviate2proc2[parent_secondary %in% unique(treeNewER$parent),]) > 0){
       
      toDeviate2proc2[parent_secondary %in% unique(treeNewER$parent), 
                      availability_secondary_primEq_tot := sum(availability_secondary_primEq, na.rm = TRUE), 
      ...
      rank2needed <- unique(secondaryFP[rank == 1 & availability_secondary_primEq  < secondLevelProcessing, 
                                        .(geographicAreaM49_fi, parent, timePointYears, parent_primary, secondLevelProcessing, availability_secondary_primEq) ])
    
  # !all(unique(rank2needed$parent_primary) %in% (secondaryFP$parent_primary))
  #     # List of solved problem with rank 1 children
  #     setkey(secondaryFP)
  #     allsolved <- unique(secondaryFP[rank == 1 & availability_secondary_primEq  >= secondLevelProcessing, ])
  #           
  #     secondaryFP[!parent_primary %in% unique(allsolved$parent_primary) & !parent_primary %in% unique(rank2needed$parent_primary) ]
  #     # If there is a case where there is no rank1 child, i.e. not everythig has been solved with rank1 then go to other ranks
  #     if(!all(unique(secondaryFP$parent_primary) %in% unique(allsolved$parent_primary))){
  #     
  #     rank2needed <- rbind(rank2needed, unique(secondaryFP[!parent_primary %in% allsolved$parent_primary &
  #                                                         availability_secondary_primEq  >= secondLevelProcessing, 
  #                                       .(geographicAreaM49_fi, parent, timePointYears, 
  #                                         parent_primary, secondLevelProcessing, availability_secondary_primEq) ]))
  #     
  #     rank2needed[, availability_secondary_primEq := 0]
  #     
  #     }
      ...
      if(nrow(rank2needed) > 0){
        ...
        secondaryFPneeded <- merge(secondaryFP[ rank == 2], rank2needed, 
                                   ...
                                   all = TRUE, suffixes = c('_available', '_needed'))
        ...
        rank3needed <- unique(secondaryFP2[rank == 2 & availability_secondary_primEq  < thirdLevelProcessing, 
                                           .(geographicAreaM49_fi, parent, timePointYears, parent_primary, thirdLevelProcessing, availability_secondary_primEq) ])
        
        # # List of solved problem with rank 1 children
        # allsolved <- unique(secondaryFP[rank == 2 & availability_secondary_primEq  >= secondLevelProcessing, ])
        # 
        # # If there is a case where there is no rank1 child, i.e. not everythig has been solved with rank1 then go to other ranks
        # if(!all(unique(secondaryFP$parent_primary) %in% unique(allsolved$parent_primary))){
        #   
        #   rank3needed <- rbind(rank3needed, unique(secondaryFP[!parent_primary %in% allsolved$parent_primary &
        #                                                          availability_secondary_primEq  >= secondLevelProcessing, 
        #                                                        .(geographicAreaM49_fi, parent, timePointYears, 
        #                                                          parent_primary, thirdLevelProcessing, availability_secondary_primEq) ]))
        #   
        #   rank3needed[, availability_secondary_primEq := 0]
        #   
        # }
        ...
        if(nrow(rank3needed) > 0){
          ...
          rank4needed <- unique(secondaryFP3[rank == 3 & availability_secondary_primEq  < fourthLevelProcessing, 
                                             .(geographicAreaM49_fi, parent, timePointYears, parent_primary, fourthLevelProcessing, availability_secondary_primEq) ])
          
          # List of solved problem with rank 1 children
          # allsolved <- unique(secondaryFP[rank == 3 & availability_secondary_primEq  >= secondLevelProcessing, ])
          # 
          # # If there is a case where there is no rank1 child, i.e. not everythig has been solved with rank1 then go to other ranks
          # if(!all(unique(secondaryFP$parent_primary) %in% unique(allsolved$parent_primary))){
          #   
          #   rank4needed <- rbind(rank4needed, unique(secondaryFP[!parent_primary %in% allsolved$parent_primary &
          #                                                          availability_secondary_primEq  >= secondLevelProcessing, 
          #                                                        .(geographicAreaM49_fi, parent, timePointYears, 
          #                                                          parent_primary, fourthLevelProcessing, availability_secondary_primEq) ]))
          #   
          #   rank4needed[, availability_secondary_primEq := 0]
          #   
          # }
