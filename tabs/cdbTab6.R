...
cdbTab_reac <- reactive({
  ...
  if(nrow(InitialDatasets$CDB) == 0){
    KeyComm <- DatasetKey(domain = domainComm, dataset = datasetCDB, dimensions = list(
      ...
      measuredItemISSCFC = Dimension(name = "measuredItemISSCFC", 
                                     keys = GetCodeList(domainComm, datasetCDB,"measuredItemISSCFC" )[,code]),
      measuredElement = Dimension(name = "measuredElement", 
                                  keys = GetCodeList(domainComm,datasetCDB, "measuredElement" )[,code]),
  ...
  # if(any(names(commodityDB) == 'ics')){
  #   setnames(commodityDB, 'ics', 'measuredItemISSCFC')}
