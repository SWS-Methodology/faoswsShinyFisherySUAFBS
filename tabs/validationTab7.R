      ...
      withProgress(message = 'Calculation in progress',
                   value = 0, {
                     ...
                     if(nrow(InitialDatasets$CDB) == 0)
                     {
                       ...
                       commodityDBAggr <- commodityDB[ , list(Value = sum(Value, na.rm = TRUE),
                       ...
                       if(nrow(tradeQ) > 0){
                         ...
                         commodityDB <- rbind(commodityDBAggr[!measuredElement %in% c('5930', '5630')], 
                         ...
                         commodityDB <- copy(commodityDBAggr)
                       }
                     ...
                     {
                       commodityDB <- rbind(InitialDatasets$CDB, InitialDatasets$CDBVal)
                       setkey(commodityDB)
                       commodityDB <- unique(commodityDB)
                     }
                     
                     # InitialDatasets$CDB <- newCDB[!measuredElement %in% ValueElements]
                     # InitialDatasets$CDBVal <- newCDB[measuredElement %in% ValueElements]
                     ...
                     SUAprimMod <- SUAunbalMod[ measuredElementSuaFbs %in% c(primaryEl, ValueElements)]
                     
                     SUAsecMod <- SUAunbalMod[!measuredElementSuaFbs %in% c(primaryEl, ValueElements)] 
                     ...
                     FPfile[['primary']] <- SUAbalResults$FPproblems$primary
                     FPfile[['secondaryTot']] <- SUAbalResults$FPproblems$secondaryTot
                     FPfile[['secondary']] <- SUAbalResults$FPproblems$secondary
                     FPfile[['tertiary']] <- SUAbalResults$FPproblems$tertiary
                     FPfile[['quaternary']] <- SUAbalResults$FPproblems$quaternary
                     FPfile[['NotCovered']] <- SUAbalResults$FPproblems$NotCovered
    ...
    {  # Closing input$reprocess == 'SUAb' & Opening input$reprocess == 'SUAbTot'
      ...
      withProgress(message = 'Calculation in progress',
                   value = 0, {
                     ...
                     #SUAcomp[!is.na(ValueMod) & Value != ValueMod | is.na(Value) ]
                     #  
                     SUAcomp <-SUAcomp[][!is.na(ValueMod) & Value != ValueMod | is.na(Value) , c('Value', 
                                                                                                  'flagObservationStatus', 
                                                                                                  'flagMethod') := list(ValueMod,
                     ...
                     SUAunbalMod <- merge(SUAunbal, SUA2replace, 
                                          ...
                                          all = TRUE, suffixes = c('','Mod'))
                     # 
                     SUAunbalMod <-  SUAunbalMod[!is.na(ValueMod) & Value != ValueMod  | is.na(Value),
                                                  c('Value', 'flagObservationStatus', 'flagMethod') := list(ValueMod, flagObservationStatusMod,
                                                                                                            flagMethodMod)]
                   ...
                   #  print(SUAbalResults$FPproblems)
                     # FPfile <<- SUAbalResults$FPproblems
                     FPfile[['primary']] <- SUAbalResults$FPproblems$primary
                     FPfile[['secondaryTot']] <- SUAbalResults$FPproblems$secondaryTot
                     FPfile[['secondary']] <- SUAbalResults$FPproblems$secondary
...
output$FPtab1 <- DT::renderDataTable({
  req(input$btn_group_fbs_tab7, input$btn_ics_prod_tab7, input$btn_sua_elem_tab7,
      input$btn_country, input$btn_year, input$btn_start_year)
  
  # feedback$FP <- FPfile
  #if(workaround$V == 1) browser()
...
output$FPinsuff <- DT::renderDataTable({
  req(input$btn_group_fbs_tab7, input$btn_ics_prod_tab7, input$btn_sua_elem_tab7,
      input$btn_country, input$btn_year, input$btn_start_year)
  ...
  if(any(names(tab1) == 'Quantity2cover')){
    setnames(tab1, 'Quantity2cover', 'quantity2cover')
  }
  if(any(names(tab1) == 'AvailableQuantity')){
    setnames(tab1, 'AvailableQuantity', 'availablequantity')
  }
...
output$FPsecPar <- DT::renderDataTable({
  req(input$btn_group_fbs_tab7, input$btn_ics_prod_tab7, input$btn_sua_elem_tab7,
      input$btn_country, input$btn_year, input$btn_start_year)
  sel_country <- country_input[country_input$label == input$btn_country, code]
  ...
  if(any(names(tab) == 'Quantity2cover')){
    setnames(tab, 'Quantity2cover', 'quantity2cover')
  }
  
  # tab <- rbind(tab, tab4)
  if(nrow(tab) > 0){
    tab[!is.na(quantity2cover), quantity2cover := round(quantity2cover,3)]
...
output$FPtabUncov <- DT::renderDataTable({
  req(input$btn_group_fbs_tab7, input$btn_ics_prod_tab7, input$btn_sua_elem_tab7,
      input$btn_country, input$btn_year, input$btn_start_year)
  sel_country <- country_input[country_input$label == input$btn_country, code]
  
  tabUncov <- FPfile$NotCovered
  tabUncov <- as.data.table(tabUncov)
  # if(is.null(tabUncov)){
  #   
  #   DT::datatable(data.table(), extensions = 'Buttons', filter = 'top',
  #                 rownames = FALSE, options = list(pageLength = 25,
