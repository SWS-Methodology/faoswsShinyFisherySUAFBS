...
SUAunbalCalc <- function(globalProductionAggr, commodityDBAggr){
  ...
  # Now not considering food processing 
...
SUAnewEr[ , c('ValueNew', 'flagObservationStatusNew', 'flagMethodNew'):=NULL]
  ...
  FPdataupd[is.na(Value) | Value != ValueRecalc | flagMethod != 'f', c('Value',
  ...
  SUAunbal <- rbind(SUAinput[!is.na(Value), ], FPdataupd)
  ...
  # Here I change from the plugin because balancing element is recalculated from scratch
  # So nothing to add to previous (possbily wrong) values
