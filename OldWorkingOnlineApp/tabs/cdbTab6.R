...
cdbTab_reac <- reactive({
  ...
  sel_ics_prod <- as.character(groups_input[label == input$btn_ics_prod_tab6]$code)
  sel_element_cdb <- as.character(element_input[ label %in% input$btn_element_cdb_tab6]$code)

  # ICS product in the chosen FBS group
  map_isscfc_filtered <- map_isscfc[ICSProd == sel_ics_prod]
...
KeyComm <- DatasetKey(domain = "Fisheries Commodities", dataset = "commodities_total", dimensions = list(
  ...
  measuredItemISSCFC = Dimension(name = "measuredItemISSCFC", keys = map_isscfc_filtered$measuredItemISSCFC),
  measuredElement = Dimension(name = "measuredElement", keys = sel_element_cdb),
...
if(nrow(cdb_map) == 0 ){
  ...
  cdb_map <-  data.table(country = '', isscfc = '', element = '', 
                         from_code = '', to_code = '', 
                         start_year = '', end_year = '', ratio = '') 
}
...
observeEvent(input$saveCDB, {
  ...
  cdb_map <- ReadDatatable('cdb_mapping', where = paste("country = '", sel_country, "'", sep = '')) 
