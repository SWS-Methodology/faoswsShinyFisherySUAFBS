...
overviewTab_reac <- reactive({
  ...
  frozenVSlive[is.na(flagObservationStatusFrozen), flagObservationStatusFrozen := 'O']
  frozenVSlive[is.na(flagObservationStatusLive), flagObservationStatusLive := 'O']
  
  frozenVSlive[is.na(flagMethodFrozen), flagMethodFrozen := '-']
  frozenVSlive[is.na(flagMethodLive), flagMethodLive := '-']
...
output$fbs_fias_tab1 <- DT::renderDataTable( server = FALSE, {
  ...
  fbs_fias_tab_out <- fbs_fias_tab_out[ , c( 'geographicAreaM49_fi', 'measuredItemFaostat_L2',
                                     'measuredElementSuaFbs', 'timePointYears',
                                     'flagObservationStatusFrozen', 'flagMethodFrozen', 
                                     'flagObservationStatusLive', 'flagMethodLive') := list(as.character(geographicAreaM49_fi),
                                                                                            as.character(measuredItemFaostat_L2),
