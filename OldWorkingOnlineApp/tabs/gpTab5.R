...
gpTab_reac <- reactive({
  ...
  sel_group_fbs <- as.character(groups_input[label == input$btn_group_fbs_tab5]$code)

  group_sel <- groups_input[ label == input$btn_group_fbs_tab5]$code
  # ICS product in the chosen FBS group
  ICSinput_code <- l2l1[code_l1 == sel_group_fbs ]$code_l2
  ...
  KeyGlobal <- DatasetKey(domain = "Fisheries", dataset = "fi_global_production", dimensions = list(
    ...
    fisheriesAsfis = Dimension(name = "fisheriesAsfis", keys = map_asfis_filtered$fisheriesAsfis),
  ...
  if(nrow(gp_map) == 0 ){
    ...
    gp_map <-  data.table(country = '', asfis = '',
                           from_code = '', to_code = '', 
...
observeEvent(input$saveGP, {
  ...
  gp_map <- ReadDatatable('gp_mapping', where = paste("country = '", sel_country, "'", sep = '')) 
